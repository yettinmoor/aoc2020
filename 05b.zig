...
pub fn main() !void {
    ...
    var ids = std.ArrayList(u10).init(allocator);
    ...
    std.sort.sort(u10, ids.items, {}, comptime std.sort.asc(u10));

    const my_id = for (ids.items[0 .. ids.items.len - 1]) |id, i| {
        const next = ids.items[i + 1];
        if (id != next - 1) {
            break id + 1;
        }
    } else unreachable;
