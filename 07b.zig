...
fn solve(rules: []const u8) !u16 {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    const allocator = &gpa.allocator;

    var bag_types = std.StringHashMap(Bag).init(allocator);

    var it = mem.split(rules, "\n");
    while (it.next()) |rule| {
        var rule_it = mem.split(rule, " ");
        const container_color = blk: {
            const color1 = rule_it.next().?;
            const color2 = rule_it.next().?;
            break :blk try mem.join(allocator, " ", &[_][]const u8{ color1, color2 });
        };
        var gop = try bag_types.getOrPut(container_color);
        if (!gop.found_existing) {
            gop.entry.value.contained = ArrayList(Bag.Contained).init(allocator);
        }
        std.debug.assert(mem.eql(u8, rule_it.next().?, "bags"));
        std.debug.assert(mem.eql(u8, rule_it.next().?, "contain"));
        while (true) {
            const amt = blk: {
                const amt_txt = rule_it.next() orelse break;
                if (mem.eql(u8, amt_txt, "no")) {
                    break;
                }
                break :blk try std.fmt.parseInt(u16, amt_txt, 10);
            };
            const contained_color = blk: {
                const color1 = rule_it.next().?;
                const color2 = rule_it.next().?;
                break :blk try mem.join(allocator, " ", &[_][]const u8{ color1, color2 });
            };
            try gop.entry.value.contained.append(.{ .amt = amt, .color = contained_color });
            _ = rule_it.next().?;
        }
    }

    return findNumberOfContained(
        bag_types,
        "shiny gold",
    );
}
...
// Recursive
fn findNumberOfContained(
    bag_types: StringHashMap(Bag),
    // found: *StringHashMap(void),
    container: []const u8,
) anyerror!u16 {
    var number: u16 = 0;
    var entry = bag_types.get(container) orelse return 0;
    for (entry.contained.items) |contained| {
        number += contained.amt * (1 + try findNumberOfContained(bag_types, contained.color));
    }
    return number;
}

const Bag = struct {
    contained: ArrayList(Contained),
...
test "test input" {
    ...
    const ans1 = try solve(test_input1);
    std.testing.expect(ans1 == 32);
    ...
    const ans2 = try solve(test_input2);
    std.testing.expect(ans2 == 126);
}
