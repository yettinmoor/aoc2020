...
fn isValid(input: []const u8) bool {
    var it = mem.split(input, " ");
    const limits = blk: {
        const limit = it.next().?;
        var limit_it = mem.split(limit, "-");
        const pos1 = fmt.parseInt(u16, limit_it.next().?, 10) catch unreachable;
        const pos2 = fmt.parseInt(u16, limit_it.next().?, 10) catch unreachable;
        break :blk .{ .pos1 = pos1, .pos2 = pos2 };
    };
    const char = it.next().?;
    const password = it.next().?;

    const pos1_ok = password[limits.pos1 - 1] == char[0];
    const pos2_ok = password[limits.pos2 - 1] == char[0];
    return (pos1_ok and !pos2_ok) or (!pos1_ok and pos2_ok);
}

test "test input" {
    std.testing.expect(isValid("1-3 a: abcde"));
    std.testing.expect(!isValid("1-3 b: cdefg"));
    std.testing.expect(!isValid("2-9 c: ccccccccc"));
}
