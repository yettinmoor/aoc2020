...
pub fn main() !void {
    const ans = solve(passports);
    const stdout = std.io.getStdOut().writer();
    try stdout.print("{}\n", .{ans});
}

fn solve(input: []const u8) u16 {
    const Field = struct {
        name: []const u8,
        validateFn: fn ([]const u8) bool,
    };
    const fields = [_]Field{
        .{ .name = "byr", .validateFn = validateByr },
        .{ .name = "iyr", .validateFn = validateIyr },
        .{ .name = "eyr", .validateFn = validateEyr },
        .{ .name = "hgt", .validateFn = validateHgt },
        .{ .name = "hcl", .validateFn = validateHcl },
        .{ .name = "ecl", .validateFn = validateEcl },
        .{ .name = "pid", .validateFn = validatePid },
    };
    const FieldInt = std.meta.Int(.unsigned, fields.len);

    var valid_count: u16 = 0;
    var fields_found: FieldInt = 0;

    var it = mem.split(input, "\n");
    while (it.next()) |line| {
        if (line.len == 0) {
            valid_count += @boolToInt(fields_found == std.math.maxInt(FieldInt));
            fields_found = 0;
        } else {
            var line_it = mem.split(line, " ");
            while (line_it.next()) |kv| {
                const key_end = mem.indexOfScalar(u8, kv, ':') orelse continue;
                const key = kv[0..key_end];
                const value = kv[key_end + 1 ..];
                inline for (fields) |field, i| {
                    if (std.mem.eql(u8, field.name, key) and field.validateFn(value)) {
                        fields_found |= 1 << i;
                        break;
                    }
                }
...
fn validateHgt(value: []const u8) bool {
    const units_index: usize = for (value) |c, i| {
        if (!std.ascii.isDigit(c)) break i;
    } else return false;
    const height = std.fmt.parseInt(u16, value[0..units_index], 10) catch return false;
    const unit = value[units_index..];
    if (mem.eql(u8, unit, "cm")) {
        return 150 <= height and height <= 193;
    } else if (mem.eql(u8, unit, "in")) {
        return 59 <= height and height <= 76;
    } else return false;
}

fn validateHcl(value: []const u8) bool {
    return value[0] == '#' and value.len == 7 and for (value[1..]) |c| {
        if (!std.ascii.isXDigit(c)) break false;
    } else true;
}

fn validateEcl(value: []const u8) bool {
    const valid_colors = [_][]const u8{ "amb", "blu", "brn", "gry", "grn", "hzl", "oth" };
    for (valid_colors) |col| {
        if (mem.eql(u8, value, col)) {
            return true;
        }
    }
    return false;
...
fn validatePid(value: []const u8) bool {
    return value.len == 9 and for (value) |c| {
...
test "test input" {
    ...
    ;
    std.testing.expect(solve(good_input) == 4);
    std.testing.expect(solve(bad_input) == 0);
}
