...
fn isValid(input: []const u8) bool {
    ...
    const limits = blk: {
        ...
        const low = fmt.parseInt(u16, limit_it.next().?, 10) catch unreachable;
        const high = fmt.parseInt(u16, limit_it.next().?, 10) catch unreachable;
    ...
    return limits.low <= count and count <= limits.high;
}
