        ...
        for (seats) |seat, i| {
            ...
            switch (seat) {
                .empty => {
                    ...
                    for (directions) |dir| {
                        if (seesNeighbor(dir, col, row, cols, rows, seats)) break;
                    } else {
                        next[i] = .occupied;
                        changed = true;
                    }
                ...
                .occupied => {
                    // >= 5 neighbors => becomes empty
                    var neighbors: u8 = 0;
                    for (directions) |dir| {
                        neighbors += @boolToInt(seesNeighbor(dir, col, row, cols, rows, seats));
                    }
                    if (neighbors >= 5) {
                        next[i] = .empty;
                        changed = true;
...
fn seesNeighbor(
    ...
    while (true) {
        x = switch (dir) {
            .w, .nw, .sw => std.math.sub(usize, x, 1) catch return false,
            .e, .ne, .se => x + 1,
            else => x,
        };
        y = switch (dir) {
            .n, .nw, .ne => std.math.sub(usize, y, 1) catch return false,
            .s, .sw, .se => y + 1,
            else => y,
        };
        if (x >= cols or y >= rows) {
