...
fn solve(data: []const u8) u16 {
    ...
    while (it.next()) |group| {
        var answers = [_]bool{true} ** 26;
        var group_it = std.mem.split(group, "\n");
        while (group_it.next()) |answered| {
            var these_answers = [_]bool{false} ** 26;
            for (answered) |c| {
                these_answers[c - 'a'] = true;
            }
            for (answers) |ans, i| {
                answers[i] = ans and these_answers[i];
            }
        }
        for (answers) |ans| {
            sum += @boolToInt(ans);
...
const input =
    ...
    \\nyliauzgfbcprewhdtjmsxoq
    \\taxnylhvgjqewcksfpmzdbior
    \\
    \\h
    \\c
    \\om
