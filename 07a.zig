...
fn solve(rules: []const u8) !u16 {
    ...
    while (it.next()) |rule| {
        ...
        while (true) {
            const amt = rule_it.next() orelse break;
            if (mem.eql(u8, amt, "no")) {
                break;
            }
            ...
            var gop = try bag_types.getOrPut(contained_color);
            if (!gop.found_existing) {
                gop.entry.value.contained_by = ArrayList([]const u8).init(allocator);
            }
            try gop.entry.value.contained_by.append(container_color);
            _ = rule_it.next().?;
        }
    }

    return findNumberOfContainers(
        bag_types,
...
// Recursive
fn findNumberOfContainers(
    bag_types: StringHashMap(Bag),
    found: *StringHashMap(void),
    contained: []const u8,
) anyerror!u16 {
    var number: u16 = 0;
    var entry = bag_types.get(contained) orelse return 0;
    for (entry.contained_by.items) |container| {
        const gop = try found.getOrPut(container);
        if (!gop.found_existing) {
            number += 1 + try findNumberOfContainers(bag_types, found, container);
        }
    }
    return number;
}
...
const Bag = struct {
    contained_by: ArrayList([]const u8),
};
